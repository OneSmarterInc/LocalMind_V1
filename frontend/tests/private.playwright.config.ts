...
export default defineConfig({
 testDir:'.',testMatch:process.env.LM_E2E_REAL_MODEL==='1'?'private-real-model.spec.ts':[ 'private-browser.spec.ts','private-parser.spec.ts' ],
 ...
 use:{...(process.env.LM_BROWSER_PROXY?{proxy:{server:process.env.LM_BROWSER_PROXY,bypass:'127.0.0.1,localhost'}}:{}),baseURL:'http://127.0.0.1:8765',viewport:{width:1365,height:900},screenshot:'only-on-failure',trace:'retain-on-failure'},
