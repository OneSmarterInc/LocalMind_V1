...
test('bundled native parser HTML recognises a scan with no network and a restrictive CSP',async({page,context})=>{
 ...
 expect(result.parsed.visuals[0].dataUrl).toMatch(/^data:image\/png;base64,/);expect(result.parsed.visuals.some((v:any)=>v.kind==='page')).toBe(false);expect(errors).toEqual([]);
});
...
test('readable illustrated PDF keeps only cropped visuals and reports progress without OCR',async({page})=>{
 await page.addScriptTag({path:'public/private-assets/parser.js',type:'module'});
 ...
 expect(result.parsed.visuals.some((v:any)=>v.kind==='page')).toBe(false);
...
test('44-page book streams preserved cropped PNGs to IndexedDB',async({page})=>{
 ...
 const result=await page.evaluate(async raw=>{
  ...
  let bytes=0,count=0,active=0,maxActive=0,pageVisuals=0;
  const parsed=await(window as any).__LM_PARSER__.parse(Uint8Array.from(atob(raw),c=>c.charCodeAt(0)),'large.pdf',undefined,()=>{},async(v:any)=>{
   active++;maxActive=Math.max(maxActive,active);bytes+=v.dataUrl.length;count++;if(v.kind==='page')pageVisuals++;
  ...
  db.close();return {bytes,count,saved,maxActive,pages:parsed.items.length,retained:parsed.visuals.length,pageVisuals};
 },fs.readFileSync('test-results/large-illustrated.pdf').toString('base64'));
 expect(result.count).toBeGreaterThan(0);expect(result.pages).toBe(44);expect(result.pageVisuals).toBe(0);
...
test('native image bridge waits for storage acknowledgement before completing',async({page})=>{
 ...
 expect(result.complete.error).toBeUndefined();expect(result.images).toBeGreaterThan(0);
