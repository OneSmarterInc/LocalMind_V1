...
export function looksLikeQrCanvas(canvas){
 ...
 const data=ctx.getImageData(0,0,tmp.width,tmp.height).data,m=[];let black=0,chromatic=0;
 for(let y=0;y<tmp.height;y++){const row=[];for(let x=0;x<tmp.width;x++){const i=(y*tmp.width+x)*4,v=(data[i]*.299+data[i+1]*.587+data[i+2]*.114)<128?1:0;row.push(v);black+=v;if(Math.max(data[i],data[i+1],data[i+2])-Math.min(data[i],data[i+1],data[i+2])>60)chromatic++;}m.push(row);}
 const ratio=black/Math.max(1,tmp.width*tmp.height);if(chromatic/(tmp.width*tmp.height)>.15){tmp.width=0;tmp.height=0;return false;}if(ratio<.12||ratio>.72){tmp.width=0;tmp.height=0;return false;}
