...
function Books({owner}:{owner:string}){
 ...
 const [rows,setRows]=useState<LocalBook[]>([]),[subjects,setSubjects]=useState<Awaited<ReturnType<LocalBooks['subjects']>>>([]),[subject,setSubject]=useState(''),[title,setTitle]=useState(''),[error,setError]=useState('');
 ...
 return <Screen><PageHeading title="Books on this device" subtitle="Import and prepare institutional books locally, then synchronize for review." right={<Button title="Books & modules" variant="secondary" onPress={()=>router.push('/manage/books')}/>}/>
 <ErrorBanner message={error||task.error}/>
 <Card><H2>Import a new book</H2><P muted>PDF or DOCX, up to 35 MB in the current local importer. Save your subject list and offline app files while connected before working offline. Imports and generation run on this device. Book synchronization sends the original file and extracted text for institutional review. Generated lessons and quizzes have separate approval actions.</P>
 <Dropdown label="Subject" value={subject} onChange={setSubject} options={subjects.map(s=>({value:s.id,label:s.code+' · '+s.name}))}/><Input label="Book title" value={title} onChangeText={setTitle}/>
 <Row><Button title="Refresh assigned subjects" variant="secondary" busy={task.busy} onPress={()=>task.run(async()=>{setSubjects(await service.subjects(true));})}/><Button title="Import on this device" disabled={!subject||!title.trim()} busy={task.busy} onPress={()=>task.run(async signal=>{
