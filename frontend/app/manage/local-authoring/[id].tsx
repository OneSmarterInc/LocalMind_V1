...
function Authoring(){
 ...
 <Card><P muted>{draft?'Source ready · Changes saved automatically':'Preparing source…'}</P><Row>{draft&&(!draft.localBook||draft.snapshot.remote_id)?<Button title="Back to outline" icon="arrow-back" variant="secondary" onPress={()=>router.push({pathname:"/manage/document/[id]",params:{id:draft.snapshot.document_id,tab:"outline",module:draft.snapshot.remote_id||draft.snapshot.module_id}})}/>:null}<Button title="Books & modules" variant="secondary" onPress={()=>router.push('/manage/books')}/></Row></Card>
