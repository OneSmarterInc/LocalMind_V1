...
function Authoring(){
 ...
 const jobs=useGenerationJobs(library?.prefix||'').filter(j=>j.bookId===id||!!draft?.snapshot.document_id&&(j.documentId===draft.snapshot.document_id||j.bookId===draft.snapshot.document_id));
 ...
 const generate=(kind:'lesson'|'quiz')=>{try{setError('');generationJobs.enqueue({scope:jobScope(library!.prefix),bookId:id,documentId:draft?.snapshot.document_id,sectionId:id,kind:'staff-'+kind,label:`${draft?.snapshot.title||'Module'} · ${kind}`},(signal,progress)=>service.generate(id,kind,signal,progress,Number(quizCount)));}catch(e){setError(String(e));}};
 ...
 <Card><P muted>{draft?'Source ready · Changes saved automatically':'Preparing source…'}</P><Row>{draft&&(!draft.localBook||draft.snapshot.remote_id)?<Button title="Back to outline" icon="arrow-back" variant="secondary" onPress={()=>router.push({pathname:"/manage/document/[id]",params:{id:draft.snapshot.document_id,tab:"outline",module:draft.snapshot.remote_id||draft.snapshot.module_id}})}/>:null}<Button title="Books & modules" icon="arrow-back" variant="secondary" onPress={()=>router.push('/manage/books')}/></Row></Card>
 ...
 {!draft.localBook||draft.snapshot.remote_id?<Button title="Prepare book" icon="arrow-forward" iconPosition="right" variant="secondary" onPress={()=>router.push(`/manage/local-batch?document=${draft.snapshot.document_id}`)}/>:null}
 ...
 {draft.lesson?<Card><H2>Review lesson</H2><LocalLessonView lesson={draft.lesson} visuals={draft.snapshot.source_visuals||localFigures.data||[]}/><Button title={draftStatus(draft,'lesson')==='Synchronized'?'Lesson synchronized':'Approve and synchronize lesson'} disabled={busy||draftStatus(draft,'lesson')==='Synchronized'} busy={task.busy} onPress={()=>task.run(async()=>{setDraft(await service.share(id,'lesson'));})}/></Card>:null}
 {draft.questions?<Card><H2>Review quiz</H2><P>{draft.questions.length} questions in this saved draft</P>
 <Row><Button title="Previous question" icon="arrow-back" disabled={questionIndex<=0} onPress={()=>setQuestionIndex(i=>i-1)}/><P>Question {Math.min(questionIndex,draft.questions.length-1)+1} of {draft.questions.length}</P><Button title="Next question" icon="arrow-forward" iconPosition="right" disabled={questionIndex>=draft.questions.length-1} onPress={()=>setQuestionIndex(i=>i+1)}/></Row>
 {draft.questions.slice(Math.min(questionIndex,draft.questions.length-1),Math.min(questionIndex,draft.questions.length-1)+1).map(q=><Card key={q.id}><P>{q.question}</P>{q.options.map((o,n)=><P key={n}>{String.fromCharCode(65+n)}. {o}{n===q.answer?' — correct':''}</P>)}<P>{q.explanation}</P><P small muted>Source: {q.quote}</P></Card>)}<Button title={draftStatus(draft,'quiz')==='Synchronized'?'Quiz synchronized':'Approve and synchronize quiz draft'} disabled={busy||draftStatus(draft,'quiz')==='Synchronized'} busy={task.busy} onPress={()=>task.run(async()=>{setDraft(await service.share(id,'quiz'));})}/></Card>:null}
