...
function Drafts({owner}:{owner:string}){
 ...
 const [rows,setRows]=useState<QuizDraft[]>([]),[ready,setReady]=useState(false),[error,setError]=useState('');
 const jobs=useGenerationJobs(library?.prefix||'');
 useEffect(()=>{let live=true;const read=async()=>{try{const [rows,status]=await Promise.all([service.list(),device().then(d=>d.status())]);if(live){setRows(rows);setReady(status.installed);}}catch(e){if(live)setError(String(e));}};void read();const timer=setInterval(read,1000);return()=>{live=false;clearInterval(timer);};},[service]);
 const generate=(row:QuizDraft)=>{try{generationJobs.enqueue({scope:jobScope(library!.prefix),bookId:row.id,sectionId:row.id,kind:'staff-quiz-selection',label:row.title},(signal,progress)=>service.generate(row.id,signal,progress));}catch(e){setError(String(e));}};
 ...
 {!ready?<Card><P>Set up your model before generating questions.</P><Button title="Offline AI" onPress={()=>router.push('/manage/offline-ai')}/></Card>:null}
