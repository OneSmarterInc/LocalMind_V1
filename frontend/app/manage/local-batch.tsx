...
function Batch({owner}:{owner:string}){
 ...
 useEffect(()=>{navigation.setOptions({backTo:id?`/manage/document/${id}?tab=outline`:"/manage/books",backLabel:id?"Back to outline":"Books & modules"});},[navigation,id]);
 ...
 useEffect(()=>{let live=true;setPreparing(true);
  void (async()=>{
   try{if(!id)throw new Error("Choose a book from Books & modules before preparing content.");const book=await manage.document(id);service.library.guard();const saved=await draftsFor(service,id);
    if(live)setShared(Object.fromEntries((book.chapters||[]).flatMap(c=>c.modules).filter(m=>m.id).map(m=>[m.id!,sharedOf(m)])));
