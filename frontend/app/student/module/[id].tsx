    ...
    <Screen refreshing={mod.loading} onRefresh={() => { mod.reload(); teach.reload(); }}>
      ...
      {m ? (
        <>
          ...
          <ModuleNav previous={m.previous_module} next={m.next_module} onGo={go} />
        </>
      ) : null}
    </Screen>
  );
  ...
  const step = (n: ModuleNeighbour | null | undefined, side: "previous" | "next") => {
    ...
    const body = (
      <View style={{ flex: 1, gap: 3, alignItems: side === "next" ? "flex-end" : "flex-start" }}>
        ...
        <Text numberOfLines={2} style={{ fontSize: 14, fontWeight: "600", color: locked ? colors.muted : colors.ink, textAlign: side === "next" ? "right" : "left" }}>
          {n.title}
        </Text>
    ...
    if (locked) {
      return (
        <View accessibilityLabel={`${n.title} is not open yet`} style={{ flex: 1, minWidth: 220, minHeight: 88, opacity: 0.55, borderWidth: 1, borderColor: colors.border, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 12, backgroundColor: "#F7F8F6" }}>
          {inner}
        </View>
      );
    }
    return (
      <Pressable accessibilityRole="button" accessibilityLabel={`Go to module ${n.number}: ${n.title}`} onPress={() => onGo(n.id)}
        style={({ pressed }) => [{ flex: 1, minWidth: 220, minHeight: 88, borderWidth: 1, borderColor: colors.border, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 12, backgroundColor: pressed ? "#F1F5EF" : colors.surface }]}>
        {inner}
      </Pressable>
    );
  };
