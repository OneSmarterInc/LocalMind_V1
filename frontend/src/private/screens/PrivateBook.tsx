...
export default function PrivateBook(){
 const {id}=useLocalSearchParams<{id:string}>(),router=useRouter(),library=useLibrary();
 ...
 useEffect(()=>{if(book.data&&!book.data.sections.some(s=>s.id===sectionId))setSectionId(book.data.sections[0].id);},[book.data,sectionId]);
 ...
 const sidebar=<Card><H2>Modules</H2><P muted>Open any module. Quiz results never lock the next one.</P><Input value={query} onChangeText={setQuery} placeholder="Find a module"/><ScrollView style={{maxHeight:550}}>
  {(b?.sections||[]).filter(x=>x.title.toLowerCase().includes(query.toLowerCase())).map((x)=><Pressable key={x.id} accessibilityRole="button" accessibilityState={{selected:x.id===sectionId}} onPress={()=>{void confirmLeave().then(ok=>{if(ok)setSectionId(x.id);});}} style={{padding:12,borderRadius:8,marginBottom:5,backgroundColor:x.id===sectionId?colors.primary:'transparent'}}><P style={{color:x.id===sectionId?'white':colors.text}}>{x.title}</P></Pressable>)}
 </ScrollView></Card>;
 return <Screen><PageHeading title={b?.title||'Private book'} subtitle="Personal study · Saved only on this device" right={<Button title="Back to library" variant="secondary" onPress={()=>{void confirmLeave().then(ok=>{if(ok)router.push('/student/private-library');});}}/>}/><ErrorBanner message={book.error} onRetry={book.reload}/>
  ...
  {b&&s&&library?<Split side={sidebar} main={<ModuleLearning key={`${library.prefix}:${id}:${s.id}`} bookId={id} section={s} next={()=>{const n=b.sections.findIndex(x=>x.id===s.id)+1;if(b.sections[n])void confirmLeave().then(ok=>{if(ok)setSectionId(b.sections[n].id);});}}/>}/>:null}
 </Screen>;
}
...
function QuizPractice({quiz}:{quiz:QuizVersion}){
 ...
 {quiz.questions.map((q,n)=><View key={q.id} style={{gap:8,paddingVertical:12,borderBottomWidth:1,borderColor:colors.border}}><H2>{n+1}. {q.question}</H2>{q.options.map((o,i)=><Pressable key={i} accessibilityRole="radio" accessibilityState={{checked:answers[q.id]===i,disabled:!ready||!!result||task.busy}} disabled={!ready||!!result||task.busy} onPress={()=>choose(q.id,i)} style={{borderWidth:1,borderColor:answers[q.id]===i?colors.primary:colors.border,padding:12,borderRadius:8,backgroundColor:answers[q.id]===i?'#EAF2ED':'white'}}><P>{String.fromCharCode(65+i)}. {o}</P></Pressable>)}{result?<Notice tone={result.checks[n].correct?'success':'warning'} title={result.checks[n].correct?'Correct':`Correct answer: ${String.fromCharCode(65+q.answer)}`} message={`${q.explanation}\nFrom the book: ${q.quote}`}/>:null}</View>)}
