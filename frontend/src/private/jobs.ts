...
export class JobQueue{
 ...
 enqueue(meta:Omit<Job,'id'|'state'|'note'|'error'>,run:NonNullable<Entry['run']>){
  ...
  if(meta.documentId&&this.entries.some(j=>j.scope===meta.scope&&j.documentId===meta.documentId&&['queued','running'].includes(j.state)))throw Error('This book is already being prepared. Wait for it to finish or cancel its current job.');
