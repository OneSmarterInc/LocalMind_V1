...
export function GenerationJobs(){const library=useLibrary(),router=useRouter(),jobs=useGenerationJobs(library?.prefix||'');
 ...
 {jobs.slice().reverse().map(j=><Card key={j.id}><Row><P>{j.label}</P><Badge value={j.state}/></Row><P muted>{j.error||j.note}</P><Row><Button title="Open book" small variant="secondary" onPress={()=>router.push(j.kind==='staff-batch'?`/manage/local-batch?document=${j.bookId}`:j.kind.startsWith('staff-')?`/manage/local-authoring/${j.bookId}`:`/student/private-book/${j.bookId}?section=${encodeURIComponent(j.sectionId)}&tab=${j.kind==='doubt'?'ask':j.kind}`)}/>{['queued','running'].includes(j.state)?<Button title="Cancel job" small variant="secondary" onPress={()=>generationJobs.cancel(j.id)}/>:null}</Row></Card>)}
 </Card>;
}
