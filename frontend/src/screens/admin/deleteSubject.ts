...
export async function deleteSubjectFlow(s: { id: string; code: string; name: string; status: string }, counts: Counts = {}) {
  ...
  const ok = await confirmDeleteAsync(`Delete ${s.name} permanently?`,
    `This removes ${books} book${books === 1 ? "" : "s"} with their modules, lessons and quizzes, ${students} enrolment${students === 1 ? "" : "s"}, every quiz attempt and score, and all faculty assignments. It cannot be undone.`,
