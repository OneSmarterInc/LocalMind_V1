...
`deploy/docker-compose.yml` brings up PostgreSQL, the API (with the embedded model mounted from `backend/models/` and the web build from `frontend/dist/`), a maintenance loop and nginx on one host; the Ollama service is behind the `ollama` compose profile and only starts with `--profile ollama` when `AI_PROVIDER=ollama`: copy `backend/.env.example` to `deploy/.env` and fill in every `CHANGE_ME` (at least `DJANGO_SECRET_KEY`, `DJANGO_ALLOWED_HOSTS`, `DJANGO_CSRF_TRUSTED_ORIGINS`, `POSTGRES_PASSWORD` and `INITIAL_USER_PASSWORD`, which is required when `DJANGO_DEBUG=false`), put your domain in `deploy/Caddyfile`, create `deploy/keys/` for the study signing key, then `cd deploy && docker compose up -d --build`. The API container migrates, collects static files and verifies (and pre-loads) the model before gunicorn starts. Uncomment the GPU block on the `ollama` service if the host has an NVIDIA card. The `web` service is Caddy, which obtains and renews the https certificate for the domain in `deploy/Caddyfile`; ports 80 and 443 must be reachable. For a plain-http campus LAN, `deploy/nginx.conf` is kept as the alternative web service.
