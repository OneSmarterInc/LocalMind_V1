...
def looks_like_qr_image(image: Image.Image) -> bool:
    ...
    if aspect > 1.38:
        return False
    # Colour photographs/textures can contain accidental finder-like runs.
    # Only apply this monochrome navigation-code heuristic to mostly neutral art.
    sample = image.convert("RGB")
    sample.thumbnail((192, 192))
    pixels = list(sample.getdata())
    if sum(max(pixel) - min(pixel) > 60 for pixel in pixels) > len(pixels) * 0.15:
        return False
