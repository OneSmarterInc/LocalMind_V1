...
def sync_document_visuals(document_id):
    ...
    if not modules or not document.file or not (document.processed_markdown_path or document.parse_mode == 'device-local'):
        return None
    ...
    try:
        processed_dir = Path(document.processed_markdown_path).parent if document.processed_markdown_path else Path(settings.MEDIA_ROOT) / "processed" / str(document.pk)
...
def document_visuals_after_processing(sender, instance, update_fields=None, **kwargs):
    if instance.status not in EDITABLE_STATUSES or not (instance.processed_markdown_path or instance.parse_mode == 'device-local'):
        return
