...
class VisualCourseIntegrationTests(TestCase):
    def test_device_book_sync_extracts_pictures_and_authoring_snapshot_keeps_them(self):
        import hashlib, json
        from uuid import uuid4
        subject=make_subject(code='DEVICEVIS');assign(self.faculty,subject)
        source=Path(self.tmp.name)/'device.docx';reference_docx(source);raw=source.read_bytes()
        manifest={'id':str(uuid4()),'subject_id':str(subject.pk),'title':'Device illustrated book',
                  'reviewed':True,'sha256':hashlib.sha256(raw).hexdigest(),'sections':[
                      {'id':'sun','title':'Sunlight','source':'Chlorophyll absorbs sunlight in the leaves to supply energy for photosynthesis.'},
                      {'id':'water','title':'Water','source':'Roots absorb water from the soil and transport it to leaves for photosynthesis.'}]}
        with self.captureOnCommitCallbacks(execute=True), patch('tutor.lessons.gateway') as gateway:
            response=client_for(self.faculty).post('/api/faculty/local-books/',
                {'manifest':json.dumps(manifest),'file':SimpleUploadedFile('device.docx',raw)},format='multipart')
            self.assertEqual(response.status_code,200,response.data)
            gateway.assert_not_called()
        document=Document.objects.get(pk=response.data['document_id'])
        self.assertEqual(document.parse_mode,'device-local')
        self.assertEqual(document.processed_markdown_path,'')
        self.assertEqual(document.visual_report['assigned'],2)
        module=Module.objects.get(pk=response.data['modules'][0]['module_id'])
        url=f'/api/faculty/modules/{module.pk}/local-authoring/'
        snapshot=client_for(self.faculty).get(url)
        self.assertEqual(snapshot.status_code,200,snapshot.data)
        self.assertEqual(len(snapshot.data['source_visuals']),1)
        self.assertEqual(snapshot.data['source_visuals'][0]['caption'],'Figure 1: Sunlight and the leaf')
        self.assertTrue(snapshot.data['source_visuals'][0]['data_url'].startswith('data:image/png;base64,'))
        self.assertNotEqual(client_for(self.other).get(url).status_code,200)
        from django.core.management import call_command
        call_command('refresh_source_visuals',document=str(document.pk),verbosity=0)
        document.refresh_from_db();self.assertEqual(document.visual_report['assigned'],2)
